...
public class ExtendedColliders3D : MonoBehaviour {
    ...
    //Enumerated Types.
    public enum ColliderType { Circle, CircleHalf, Cone, ConeHalf, Cube, Cylinder, CylinderHalf, Quad, Triangle };
    ...
    void OnDrawGizmosSelected() {
        ...
        //Draw the mesh.
        Gizmos.DrawWireMesh(generateMesh(true));
    }
    ...
    Mesh generateMesh(bool applyTransform) {
        ...
        for (int i = 0; i < vertices.Length; i++) {
            vertices[i].x *= properties.size.x * (applyTransform ? transform.localScale.x : 1);
            vertices[i].y *= properties.size.y * (applyTransform ? transform.localScale.y : 1);
            vertices[i].z *= properties.size.z * (applyTransform ? transform.localScale.z : 1);
            vertices[i] = Quaternion.Euler(properties.rotation) * vertices[i];
            vertices[i] += properties.centre;
            if (applyTransform) {
                vertices[i] = Quaternion.Euler(transform.localEulerAngles) * vertices[i];
